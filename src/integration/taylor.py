    ...
    return (
        torch.stack(
            [
                ...
                beta[3] * ds[0]
                + (beta[0] * beta[2] + beta[1] ** 2 / 2.0) * ds[1]
                + beta[0] ** 2 * beta[1] * ds[2] / 2.0
                + beta[0] ** 4 * ds[3] / 24.0,
                beta[4] * ds[0]
                + (beta[0] * beta[3] + beta[1] * beta[2]) * ds[1]
                + (beta[0] ** 2 * beta[2] + beta[0] * beta[1] ** 2) * ds[2] / 2.0
                + beta[0] ** 3 * beta[1] * ds[3] / 6.0
                + beta[0] ** 5 * ds[4] / 120.0,
            ]
        )
        .squeeze()
...
class TaylorInt(nn.Module):
    ...
    def step(self, new_width):
        ...
        self.intervals = torch.arange(self.x_min, self.x_max + self.width / 2.0, self.width, dtype=torch.float32).to(
            self.device
        )
