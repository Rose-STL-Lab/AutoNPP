...
def train(model, dataloader):
    ...
    for epoch in trange(0, 50):
        ...
        for i, (x, integrants, targets) in enumerate(dataloader):
            ...
            d2f_anchor = torch.tensor([[np.pi / 2.0], [3 * np.pi / 2.0]]).to(
                device
            ...
            loss = (
                loss_func(model.d3forward(x.unsqueeze(-1), [0, 0, 0]).squeeze(), integrants)
...
def main():
    ...
    model = MultSequential(
        nn.Linear(1, 128), nn.Tanh(), nn.Linear(128, 128), nn.Tanh(), nn.Linear(128, 128), nn.Tanh(), nn.Linear(128, 1)
    ).to(device)
