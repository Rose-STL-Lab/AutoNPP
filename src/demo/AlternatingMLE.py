...
def lamb_func(t, his_t):
    ...
    lamb_t = mu + alpha * (np.sum(np.exp(-beta * dt0)) + np.sum((c2 * dt1**2 + c1 * dt1 + c0) * np.exp(-zeta * dt1)))
    lamb_t = abs(lamb_t)
    L = 2 / lamb_t
...
def nll(his_t, alpha, beta, mu, zeta, c0, c1, c2):
    ...
    exc_delta_t[:, ~exc_mask] = 0.0
    inh_delta_t[:, exc_mask] = 0.0

    exc_lamb = torch.sum(torch.tril(torch.exp(-beta * exc_delta_t), -1)[:, exc_mask], -1)
    inh_lamb = torch.sum(
        torch.tril((c2 * inh_delta_t**2 + c1 * inh_delta_t + c0) * torch.exp(-zeta * inh_delta_t), -1)[:, ~exc_mask],
        -1,
    )
    ...
    term_2 -= alpha / beta * torch.sum((torch.exp(-beta * (t_end - his_t[exc_mask])) - torch.tensor(1.0)))
    temp = t_end - his_t[~exc_mask]

    term_3 = (
        torch.sum(
            torch.exp(-zeta * temp)
            * (-c2 * (zeta * temp * (zeta * temp + 2) + 2) - zeta * (c1 * zeta * temp + c1 + c0 * zeta))
            + (2 * c2 + zeta * (c1 + c0 * zeta))
        )
        / zeta**3
    )
    term_3 *= alpha
...
def main():
    trainset = TPPWrapper(lamb_func, n_sample=1, t_end=10000.0, max_lamb=100.0, fn="data/temporal/alterHawkes.db")
    his_t = torch.tensor(trainset.seqs[0].numpy()).to(device)
    print(nll(his_t, 0.2, 0.2, 0.2, 1.0, -1.0, 1.0, 1.0))
